...
export class TmaChecklistActionService {
  ...
  getChecklistActionForProductCode(baseSiteId: string, productCode: string): Observable<TmaChecklistActionModel.TmaChecklistAction[]> {
    this.store.select(TmaChecklistActionSelectors.getAllChecklistActions)
      .pipe(
        tap(checklistActions => {
            if (!checklistActions.find(checklistAction => checklistAction.productId == productCode && checklistAction.baseSiteId == baseSiteId)) {
    ...
    return this.store.select(TmaChecklistActionSelectors.getChecklistActionForProductCode, {productCode: productCode, baseSiteId: baseSiteId});
  }
